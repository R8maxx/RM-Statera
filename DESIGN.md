...
- **Columnas ancladas pegadas al margen izquierdo** con desplazamiento medido, no calculado: la mitad de las columnas no declara ancho. La última anclada lleva `border-r` para que se vea dónde acaba el bloque fijo.
- Filas con `border-b`, hover `bg-muted/50`, seleccionada `bg-accent/40`. Sin cebra. Cifras a la derecha con la clase `.cifra` (`tabular-nums`).

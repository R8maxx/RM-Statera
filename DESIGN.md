...
- **La columna lateral, siempre en el mismo orden**: «Estado» —el cambio de estado— arriba, «Ficha» —los metadatos, siempre con ese nombre— después, y el resto detrás. El histórico va en la columna principal, que es donde cabe leerlo. Se buscaba el botón de estado en cinco sitios distintos según el módulo. Excepciones escritas en su fichero: el incidente cierra junto a la lección aprendida, la prueba de continuidad no tiene cambio de estado genérico, el documento se aprueba bajo su borrador y el BIA lleva estado e histórico en la misma tarjeta, «Ciclo».
...
- **Dos tablas, y se elige por lo que hay que hacer con las filas.** Lo que se pagina, se filtra o se ordena en servidor va en `DataTable` sobre un `Recurso`. Lo que son unas pocas filas fijas —los sistemas bajo el ENS, los doce tipos de plantilla— va en `ui/table`, la **tabla estática**, que se pinta igual (envoltorio, cabecera en versalitas, hover sin alfa) y no fabrica un `MetaTabla` para fingir que pagina. Lo que no vale para ninguna de las dos son tarjetas iguales apiladas.
...
- **Dos focos, y sólo dos.** Lo que se pulsa —botones, enlaces, conmutadores, pestañas, chips de filtro, nodos del grafo— hereda el contorno global de `app.css` y **no lleva anillo propio**: había quince componentes con su `ring-2 … outline-none` a mano. Lo que se escribe o se elige —campos, `select` y `textarea` nativos, disparadores de filtro, el editor de texto— lleva el anillo de campo de §9 (`border-ring` + `ring-3 ring-ring/50`).
- **Las superficies del navegador también son del producto**: la selección de texto, el cursor de escritura, las casillas y fechas nativas (`accent-color`) y las barras de desplazamiento van en teal y neutros desde `app.css`. Con los valores del sistema salían en un azul que es el de `estado-planificado`.
